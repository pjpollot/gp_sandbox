...
class GPBCTest(unittest.TestCase):
    ...
    def test_likelihood(self):
        ...
        self.assertAlmostEqual(gpc.log_marginal_likelihood(), gpc_check.log_marginal_likelihood(), places=2, msg='log-marginal-likelihood are not equal')

    def __assertMatrixAlmostEqual(self, M1, M2, n_decimals):
        n, p = M1.shape
        for i in range(n):
            for j in range(p):
                self.assertAlmostEqual(M1[i,j], M2[i,j], n_decimals, 'Kernels at index (%d,%d) are not equal' % (i+1, j+1))
